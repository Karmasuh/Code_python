...
def getPokemonInfo(name):
    ...
    if response.status_code == 200:
        ...
        print(f"failed to retrevie {response.status_code}")
...
# api for ability, experience, what moves pokemon get at what levels
class Pokemon:
    ...
    def levelUp(self):
        if self.exp >= (0.8*(self.maxExp**3))//1:
            return self.level + 1
        else: 
            return self.level
...
class Moves:
    ...
    def moveDmgCalculator(self, attacker, receiver):
        ...
        damage = (((2 * level // 5) * self.power * attacker.atk // defence) // 50 + 2) * random.randrange(0.85, 1)
        ...
        if hitOrMiss <= self.accuracy:
            if self.typing == 'Physical':
                if condition == 'Burned':
                    ...
                    if type 
                return damage
            if self.typing == 'Special':
                damage = (((2 * level // 5) * power * spAtk // spDefence) // 50 + 2) * random.randrange(0.85, 1)
                return damage
...
class Combat:
    ...
    def turnOrder(self, pokemon1, pokemon2):
...
def team (pokemon1, pokemon2 == False, pokemon3 == False, pokemon4 == False, pokemon5 == False):

existingPokemon = {
    'Treecko': (40, 45, 35, 65, 55, 70, "overgrow"), #grass
    'Grovyle': (50, 65, 45, 85, 65, 95, "overgrow"), #grass
    'Sceptile': (70, 85, 65, 105, 85, 120, "overgrow"), #grass
...
#     return typeChart[attacker_type].get(defender_type, 1)

# # Example usage
# attacker = 'Fire'
# defender = 'Grass'
# effectiveness = type_effectiveness(attacker, defender)
# print(f"{attacker} vs {defender}: {'Super effective!' if effectiveness > 1 else 'Not very effective.' if effectiveness < 1 else 'Neutral'}")


existingMoves = {
'Air Cutter': ('Flying', 'Special', 60, 95, 25),  # High critical hit ratio.
'Arm Thrust': ('Fighting', 'Physical', 15, 100, 20, True, 2, 5),  # Hits 2-5 times in one turn.
...
'Bullet Seed': ('Grass', 'Physical', 25, 100, 30, True, 2, 5),  # Hits 2-5 times in one turn.
...
'Icicle Spear': ('Ice', 'Physical', 25, 100, 30, True, 2, 5),  # Hits 2-5 times in one turn.
...
'Rock Blast': ('Rock', 'Physical', 25, 90, 10, True, 2, 5),  # Hits 2-5 times in one turn.
...
'Barrage': ('Normal', 'Physical', 15, 85, 20, True, 2, 5),  # Hits 2-5 times in one turn.
'Barrier': ('Psychic', 'Status', 0, 100, 20),  # Sharply raises user's Defense.
'Bind': ('Normal', 'Physical', 15, 85, 20, True, 2, 5),  # Traps opponent, damaging them for 4-5 turns.
...
'Double Slap': ('Normal', 'Physical', 15, 85, 10, True, 2, 5),  # Hits 2-5 times in one turn.
...
'Fire Spin': ('Fire', 'Special', 35, 85, 15, True, 2, 5),  # Traps opponent, damaging them for 4-5 turns.
...
'Fury Attack': ('Normal', 'Physical', 15, 85, 20, True, 2, 5),  # Hits 2-5 times in one turn.
...
'Pin Missile': ('Bug', 'Physical', 25, 95, 20, True, 2, 5),  # Hits 2-5 times in one turn.
